...
file_path = r'C:\Users\cente\sparkfare\antigravity_build_plan.md'
with open(file_path, 'r', encoding='utf-8') as f:
    old_content = f.read()

def extract_section(title_substring):
    # Find the section starting with ### X. title_substring
    # and ending at the next ### or end of string
    pattern = r"(### \d+\.\s.*?" + re.escape(title_substring) + r".*?)(?=\n### \d+\. |\Z)"
    match = re.search(pattern, old_content, re.DOTALL)
    if match:
        return match.group(1).strip()
    return f"ERROR: Could not find {title_substring}"

t0_text = extract_section("T0 — Instrumentation and metrics baseline")
t1_text = extract_section("T1 — Price-data honesty guardrails")
t7_text = extract_section("T7 — Email deliverability and consent")
t4_text = extract_section("T4 — Share-image generator")
t2_text = extract_section("T2 — Away Mode partner registry")
t3_text = extract_section("T3 — Referral loop v1")
t6_text = extract_section("T6 — Embeddable widget")

# Strip the leading "### X. " so we can format it however we want, but let's just use "### T..." 
def strip_header(text):
    return re.sub(r"^### \d+\.\s*", "### ", text)

new_content = """# Antigravity build instructions v2 — aligned to the full GTM/monetization strategy (2026-09-22)
...
new_content += strip_header(t0_text) + "\n\n"
new_content += strip_header(t1_text) + "\n\n"
new_content += strip_header(t7_text) + "\n\n"
new_content += strip_header(t2_text) + "\n\n"

new_content += """### T13 — Secondary flight-data source (de-risking)
...
new_content += strip_header(t4_text) + "\n\n"

new_content += """### T5 — Programmatic route pages (dual-pillar version)
...
new_content += strip_header(t3_text) + "\n\n"
new_content += strip_header(t6_text) + "\n\n"

new_content += """### T7b — Web push notification channel
