...
- `<link rel="canonical" href="https://sparkfare.com/">` (also collapses the `?ref=` and `?origin=`
  query-string variants onto one URL for search engines).
- `og:image` (+ width/height/alt) and `og:site_name`, plus `twitter:card` (`summary_large_image`),
  `twitter:title`/`description`/`image`.
- JSON-LD (`Organization` + `WebSite`, linked by `@id`). Deliberately minimal: no `sameAs` (no
  confirmed social profile URLs to cite) and no `SearchAction` (the site has no search).

**The share image is new: `og-default.png` (1200x630)**, generated by
`make_og_default_image.py` (repo root, `*.py` is asset-ignored so it doesn't ship) from the brand
mark's exact geometry, the style guide's Paper/Ledger palette and the current tagline. Set in Inter
Medium (the committed `src/assets/Inter-Medium.ttf`) — Space Grotesk, the guide's headline face,
isn't available locally as a font file, so this is a close raster approximation, not a pixel match.
**Re-run the script if the tagline or mark changes**; the image bakes the tagline in. Do not use
`deal_card.jpg` for this: it is untracked output of `Phase 3 Social Broadcaster.py` (one specific
deal, 1080x1080) and is a 404 in production.
...
300 KB that isn't asset-ignored, and that the JSON-LD parses. Full suite 210/210 (`t7b_push`
excluded). A local browser load showed the deal cards, hero price and image all still rendering.
...
**A genuine Gmail open was then confirmed (2026-09-26 13:33:56 UTC).** A second test digest sent at 13:30 and opened by Coby in Gmail (with "Always display external images" already on) produced a new `email_open` event for that email's own id and advanced the account's `last_opened_at` to the same moment. This closes the caveat above: the full chain -- Resend open tracking, the `links.sparkfare.com` pixel, the Svix-signed webhook, and the D1 write -- works for a real open, not just a fetched pixel. Worth knowing: the *first* test email's Gmail open (~13:03, while the webhook was still 401ing) was never recorded -- only the two synthetic pixel fetches show up for it -- so opens that happened before 2026-09-26 13:18 UTC are permanently missing, not merely delayed. Nobody is affected today (3 test users), but it is why `last_opened_at` history starts there. Also note the hidden 1x1 pixel is never visible, so "I saw no images in the email" is expected and says nothing about whether it loaded.
